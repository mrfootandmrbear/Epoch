...
`TerrainHistory` owns inherited geology and ecological deposits: elevation, disturbance, vegetation protection, forage, nutrients, runoff, basalt, ash, volcanic load, substrate age, sediment, and carbonate. `EnvironmentField` is derived per landing and is not serialized. Material palette mixing, procedural detail, water optics, fog, and LOD remain rendering state.
