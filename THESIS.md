...
- **Stylized is allowed; arbitrary is not.** Epoch does not owe photorealism. It can simplify forms, exaggerate silhouettes, and use authored color and proportion to make a beautiful, coherent world—especially when that makes change across a jump legible. But its creatures are populations that naturally evolved in response to *this island*, not collectible fantasy variants. Stylization should clarify plausible adaptation: limb proportions shaped by terrain, feet shaped by inundation or substrate, insulation and body mass shaped by climate, coloration shaped by habitat, and feeding structures shaped by available niches. A player should be able to look at a descendant and make a reasonable guess about where and how it lives.
- **Creature asset direction — retain the Foxel workflow.** Habitat's chosen [Foxel](https://github.com/elliottdehn/foxel) pipeline remains a good fit for Epoch's stylized, zoomable fauna: author and iterate in `.fxl`, preview offline, export rigged/animated `.glb`, and ship only the resulting assets. Foxel is build-time tooling, never the runtime evolution model. Because it does not provide blend shapes or topology-stable morph targets, continuous evolved traits should drive ordinary glTF skeleton proportions at runtime, while genuinely discrete adaptations select among a small family of pre-baked Foxel variants. A shared base rig and palette should keep descendants visibly related even as island pressures pull their silhouettes apart. Close-up rendering raises the bar beyond Habitat's population markers: individuals may be ephemeral samples of a population, but they must still feel alive through animation, behavior, grounding, and readable anatomy.
...
- **Vegetation/tree rendering tooling — ez-tree evaluated, integration shape identified.** [ez-tree](https://github.com/dgreenheck/ez-tree) is a strong candidate for the *geometry generator*, not a drop-in rendering stack. Its seeded skeleton, presets, raw `createGeometry()` branch/leaf output, and shared-skeleton LOD generation fit Epoch well; branch length/radius/angle/children/gnarliness/force and leaf count/size/angle map naturally onto habitat- and trait-driven plant morphology. Its stock leaf wind uses WebGL GLSL injection through `MeshStandardMaterial.onBeforeCompile`, which does not belong in Epoch's WebGPU/TSL pipeline. If adopted, keep ez-tree's generation and LOD seam, replace its materials and wind with Epoch-owned TSL node materials, supply our own textures, and instance/cache geometry by morphology class rather than constructing one unique full tree per occupant. MIT licensing permits this adaptation. Evaluated 2026-08-09 against the repository's current `main`; the repo source/changelog contains post-1.1 LOD work while its package manifest still reports 1.1.0, so pin a commit rather than assuming the npm package contains the reviewed API.
