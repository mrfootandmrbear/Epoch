...
- **Creature asset direction — morph targets for evolved traits.** Per-instance blend weights read from a `morphTexture` (DataArrayTexture) are the chosen mechanism for expressing evolved traits on creature meshes under `WebGPURenderer`. `InstancedMesh` and `SkinnedMesh` are mutually exclusive in the current Three.js WebGPU path, and two of the seven trait axes — foot width and horn length — have no natural joint-transform expression, making per-instance skeletons a poor fit regardless. Five morph channels cover the continuous shape space (body mass, leg length, foot width, insulation, horn length); coat warmth and coat lightness are per-instance scalar floats rather than geometry channels. Walk cycle runs as pose morphs blended per instance — one draw call per herd. [Foxel](https://github.com/elliottdehn/foxel) is an early candidate for animal authoring, not a pipeline commitment: because it re-meshes on topology change, any topology-stable export with a consistent vertex count can drive morph targets, and Foxel is one option among several. A shared base mesh and palette should keep descendants visibly related even as island pressures pull their silhouettes apart. Close-up rendering raises the bar beyond Habitat's population markers: individuals may be ephemeral samples of a population, but they must still feel alive through animation, behavior, grounding, and readable anatomy.
...
**OD-1 — Morph targets vs. skeleton proportion morphing — RESOLVED.** Morph targets confirmed for Apple Silicon WebGPU. Per-instance blend weights via `morphTexture` (DataArrayTexture); `InstancedMesh` + `SkinnedMesh` are mutually exclusive in the current Three.js WebGPU path. Asset pipeline (including Foxel) is not a binding constraint — any topology-stable mesh can drive morph targets; see §6.
