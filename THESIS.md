...
- **Creature asset direction — topology-stable, tool-agnostic fauna.** Epoch needs one shared creature form that can express continuous evolved traits per visible individual while keeping descendants visibly related. Three.js currently exposes a promising `InstancedMesh` morph-target path, while Foxel remains one possible authoring tool rather than a pipeline commitment. The exact expression and animation architecture remains an implementation decision: it must first prove trait extremes, motion, grounding, herd-scale performance, and gameplay-distance readability in Epoch's `WebGPURenderer`. Close-up rendering raises the bar beyond Habitat's population markers: individuals may be ephemeral samples of a population, but they must still feel alive through animation, behavior, grounding, and readable anatomy.
