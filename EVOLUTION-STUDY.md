...
| Species: ALRE | Ecosystem | Epoch (current) |
|---|---|---|
| Real-time phylogenetic trees. Populations that drift far enough apart in trait space branch into new species on the tree. Allopatric speciation via geographic isolation. | N/A for land. | **Partial.** Two fixed identities (sheltered-grazer, ridge-grazer). `populationTraitDistance` exists to measure divergence, but no branching logic yet — lineages can go active/extinct but not split. |

> **Key lesson from Species:** Species proves that readable speciation needs visible trait variation and a phylogenetic record. Epoch already has the trait vector and lineage history; the gap is that lineages can't branch. The `populationTraitDistance` function is there — when trait drift exceeds a threshold, a lineage should fork. The per-individual genome approach from Species is too granular for Epoch's jump-based time model, but the population-level blending already does the equivalent work at the right timescale.
...
**Water domain (scaffolded):** Needs a marine trait vector (streamlining, size, depth preference, thermal tolerance), `coastalProductivity` as the energy source, wave energy from the FFT ocean as environmental stress, and a trophic layer (even a simple "small eats plankton, large eats small" chain).
...
| **Speciation trigger** | Continuous drift measured against trait distance. Speciation is gradual and always happening. | Not a focus — species are more like niches that get filled. | **Threshold on trait distance after a jump.** A big jump across a climate shift could produce a speciation event in one step. The `populationTraitDistance` function is the mechanism — it just needs a threshold and a branch action. |
...
**Species' biggest gift to Epoch is the phylogenetic tree** — the idea that speciation is a visible, trackable event the player can read, not just a hidden state change. Epoch's lineage history system is already most of the way there; it needs branching and a visualization.
